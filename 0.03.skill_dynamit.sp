...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("OdpornoscNaDynamit", native_OdpornoscNaDynamit);
...
public Action:PolozDynamit(client, args)
{
	...
	{
		if(!IsClientInGame(i) || GetClientTeam(client) == GetClientTeam(i) || OdpornoscKlasa[i] || OdpornoscItem[i])
			continue;
		if(!IsPlayerAlive(i))
			continue;
		...
		{
			//obrazenia
			new Float:damage = 80.0 + float(Cmod_GetINT(client));
...
public native_OdpornoscNaDynamit(Handle:plugin, numParams)
{
	new client = GetNativeCell(1);
